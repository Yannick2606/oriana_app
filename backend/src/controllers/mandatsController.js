...
export function createMandatsController(service) {
  return {
    async list(request, response, next) {
      try {
        const records = await service.list(request.accessScope);
    ...
    async get(request, response, next) {
      try {
        return response.status(200).json({
          data: serialize(await service.get(request.params.id, request.accessScope)),
        });
      } catch (error) { return handle(error, response, next); }
    },
    ...
    async update(request, response, next) {
      try {
        const record = await service.update(request.params.id, request.body, request.accessScope);
        return response.status(200).json({ data: serialize(record) });
      } catch (error) { return handle(error, response, next); }
    },
    async delete(request, response, next) {
      try {
        await service.delete(request.params.id, request.accessScope);
