...
export function createUtilisateursController(service) {
  return {
    async list(request, response, next) {
      try { return response.status(200).json({ data: await service.list() }); } catch (error) { return handle(error, response, next); }
    },
    async create(request, response, next) {
      try { return response.status(201).json({ data: await service.create(request.body) }); } catch (error) { return handle(error, response, next); }
    },
