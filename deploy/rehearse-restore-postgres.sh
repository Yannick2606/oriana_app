...
usage() {
  echo "Usage: T30_REHEARSAL_CONFIRM=YES rehearse-restore-postgres.sh /backups/oriana-TIMESTAMP.dump oriana_restore_t30_SUFFIXE t30_YYYYMMDD_IDENTIFIANT" >&2
  exit 2
}
...
printf '%s' "$rehearsal_id" | grep -Eq '^t30_[0-9]{8}_[a-z0-9]{6,32}$' || {
  echo "Refus : l'identifiant doit respecter t30_YYYYMMDD_[a-z0-9]{6,32}." >&2
  exit 6
}

[ -f "$dump_path" ] || { echo "Refus : archive absente." >&2; exit 7; }
...
case "$target_db" in
  postgres|template0|template1) echo "Refus absolu : base système interdite." >&2; exit 9 ;;
esac
...
psql --host "$POSTGRES_HOST" --username "$POSTGRES_USER" --dbname "$target_db" --set=ON_ERROR_STOP=1 \
  --command 'CREATE TABLE public.t30_rehearsal_marker (rehearsal_id text PRIMARY KEY, created_at timestamptz NOT NULL DEFAULT now())' \
  --command "INSERT INTO public.t30_rehearsal_marker (rehearsal_id) VALUES ('$rehearsal_id')"
...
echo "Identifiant de répétition : $rehearsal_id"
