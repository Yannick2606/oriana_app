...
function Dashboard({ user, onHelp, onNavigate, onCreateOpportunity }) {
  ...
  return <div className="space-y-7 animate-enter">
    ...
    <Card className="relative overflow-hidden border-oriana-lavande/30 bg-gradient-to-r from-[#0B050E] via-[#1A0E24] to-[#3D1E4D] text-white">
      <div aria-hidden="true" className="absolute inset-0 opacity-40 [background:radial-gradient(circle_at_80%_20%,rgba(206,147,216,.22),transparent_30%)]"/>
      <div className="relative grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-oriana-lavandeMoyen">Votre priorité</p><h2 className="mt-3 font-titre text-2xl sm:text-3xl">Un parcours clair, de la donnée à l’action.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-oriana-lavandeClair/75">Commencez par structurer l’information, qualifiez le besoin, puis laissez orIAna faire ressortir la prochaine étape utile.</p></div><Button onClick={() => onNavigate('patrimoine')}>Commencer<ArrowRight size={16}/></Button></div>
    </Card>
    <div aria-label="Parcours recommandé" className="grid gap-3 lg:grid-cols-3">
      {journey.map(({ number, title, description, action, icon: Icon, page }) => <button type="button" key={number} onClick={() => onNavigate(page)} className="group rounded-oriana border border-oriana-bordure bg-oriana-surface p-5 text-left shadow-oriana transition hover:-translate-y-0.5 hover:border-oriana-lavande"><div className="flex items-center justify-between"><span className="text-xs font-bold tracking-[.18em] text-oriana-lavande">ÉTAPE {number}</span><span className="rounded-full bg-oriana-violet/15 p-2 text-oriana-lavande"><Icon size={17}/></span></div><h2 className="mt-4 font-titre text-xl">{title}</h2><p className="mt-2 min-h-10 text-sm leading-5 text-oriana-discret">{description}</p><span className="mt-4 flex items-center gap-2 text-xs font-bold text-oriana-lavandeMoyen">{action}<ArrowRight size={14} className="transition group-hover:translate-x-1"/></span></button>)}
    </div>
