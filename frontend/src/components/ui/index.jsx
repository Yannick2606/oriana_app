...
export function Button({ variant = 'primary', size = 'md', className, children, ...props }) {
  return <button type="button" className={cx('inline-flex items-center justify-center gap-2 rounded-oriana font-semibold transition duration-oriana disabled:cursor-not-allowed disabled:opacity-50', variants[variant], size === 'sm' ? 'h-8 px-3 text-xs' : 'h-10 px-4 text-sm', className)} {...props}>{children}</button>;
}
...
const control = 'w-full rounded-oriana border border-oriana-bordure bg-oriana-fond px-3 py-2 text-sm text-oriana-texte placeholder:text-oriana-discret/70 transition focus:border-oriana-lavande focus:outline-none';
...
export function PageHeader({ eyebrow, title, description, actions }) { return <header className="flex flex-col justify-between gap-4 xl:flex-row xl:items-end"><div className="min-w-0"><p className="mb-2 text-xs font-bold uppercase tracking-[.18em] text-oriana-lavande">{eyebrow}</p><h1 className="font-titre text-3xl tracking-tight md:text-4xl">{title}</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-oriana-discret">{description}</p></div>{actions && <div className="flex flex-wrap gap-2 xl:shrink-0 xl:justify-end">{actions}</div>}</header>; }
