...
export function FormationExperience({ user, pageVisible, readOnly = false, client = formationApi }) {
  ...
  async function save(next) {
    ...
    try { setError(''); const { data } = await client.update(next); setProgress(data.progression); } catch { setError('La progression n’a pas pu être enregistrée.'); }
  }
  ...
  return <>
    {pageVisible && <div className="space-y-7 animate-enter"><Breadcrumb items={['Accueil', 'Auto-formation']}/><PageHeader eyebrow="Documentation" title="Auto-formation" description="Retrouvez à tout moment le parcours correspondant à votre rôle actif."/>
      {readOnly && !open && <p role="status" className="text-sm text-oriana-lavandeClair">Prévisualisation : progression non enregistrée.</p>}
    ...
    <Modal open={open && Boolean(step)} onClose={skip} title={step?.titre || 'Auto-formation'}><p className="text-sm leading-6 text-oriana-discret">{step?.texte}</p><p className="mt-4 text-xs text-oriana-discret">Étape {index + 1} sur {steps.length}</p>{readOnly && <p role="status" className="mt-3 text-sm text-oriana-lavandeClair">Prévisualisation : progression non enregistrée.</p>}{error && <p role="alert" className="mt-3 text-sm">{error}</p>}<div className="mt-5 flex justify-between gap-2"><Button variant="ghost" onClick={skip}>Passer</Button><Button onClick={next}>{index >= steps.length - 1 ? 'Terminer' : 'Suivant'}</Button></div></Modal>
  </>;
}
