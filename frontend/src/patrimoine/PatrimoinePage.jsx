...
function RecordButton({ record, active, onClick, icon: Icon, subtitle }) {
  return <button type="button" onClick={onClick} aria-pressed={active} className={`w-full rounded-oriana border p-3 text-left transition ${active ? 'border-oriana-violet bg-oriana-violet/10 shadow-oriana' : 'border-oriana-bordure bg-oriana-surface hover:bg-oriana-surfaceAlt'}`}>
...
function HeritageForm({ resource, record, selection, data, pending, onCancel, onSave }) {
  ...
  return <div className="space-y-5">
    ...
    {parentRequired && <p role="alert" className="text-sm font-medium text-oriana-violet">Sélectionnez un parent pour ce {title}.</p>}
